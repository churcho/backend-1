defmodule Core.ProviderView do
  ...
  def render("index.json", %{providers: providers}) do
    %{
      ...
      data: render_many(providers, Core.ProviderView, "provider.json")
    }
  end
  ...
  def render("show.json", %{provider: provider}) do
    ...
    %{
      data: render_one(provider, Core.ProviderView, "provider.json")
     }
  end
  ...
  def render("provider.json", %{provider: provider}) do
    ...
    %{
      ...
      id: provider.id,
      included: %{
       services: render_many(provider.services |> Core.Repo.preload([:provider, :entities]), Core.ServiceView, "service.json")
      },
      attributes: %{
        name: provider.name,
