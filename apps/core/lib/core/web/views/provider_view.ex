defmodule Core.Web.ProviderView do
  use Core.Web, :view
  ...
  def render("index.json", %{providers: providers}) do
    %{
      ...
      data: render_many(providers, Core.Web.ProviderView, "provider.json")
    }
  end
  ...
  def render("show.json", %{provider: provider}) do
    ...
    %{
      data: render_one(provider, Core.Web.ProviderView, "provider.json")
     }
  end
  ...
  def render("provider.json", %{provider: provider}) do
    ...
    %{
      ...
      id: provider.id,
      included: %{
       services: render_many(provider.services |> Core.Repo.preload([:provider, :entities]), Core.Web.ServiceView, "service.json")
      },
      attributes: %{
        name: provider.name,
